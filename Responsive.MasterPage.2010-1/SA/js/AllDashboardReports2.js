...
CKO.DASHBOARDS.ALLDASHBOARDS.VARIABLES = {
    ...
    '       <td>Time Period:&nbsp;&nbsp;</td>' +
    '       <td>' +
    '         <select class="form-control form-control-sm" id="TimeFilter" name="Time Period" onchange="CKO.DASHBOARDS.ALLDASHBOARDS.Reports().ChangeMe(this);">' +
    ...
    '       <td>' +
    '          <select class="form-control form-control-sm" id="PersonTypeFilter" name="Personnel Type">' +
    ...
    '       <td>&nbsp;&nbsp;Organization:&nbsp;&nbsp;</td>' +
    '       <td>' +
    '            <select class="form-control form-control-sm" id="OrgFilter" name="Organization">' +
    '                <option id="All" selected value="All">All</option>' +
...
CKO.DASHBOARDS.ALLDASHBOARDS.Reports = function () {
    ...
    function drawBaseUI() {
        ...
        for (var i = 0; i < v.charts.length; i++) {
            ...
            v.html += "<div class='card maincard card-" + chart + "'>";
            v.html += "     <div class='card-header'>";
            v.html += "         <h4><button class='btn btn-link' type='button' data-id='" + chart + "' data-toggle='collapse' data-target='#collapse_" + chart + "'>" + title + "</button></h4>";
            v.html += "     </div>";
            v.html += "     <div id='collapse_" + chart + "' data-chart=" + chart + " class='collapse' data-drawn='false' data-parent='#AllDashboardReports' data-index='" + chart + "'>";
            v.html += "         <div class='card-body'>";
            v.html += "             <div class='card'>";
            v.html += "                 <div class='card-header'>";
            v.html += "                     <table class='allDashboardFilter'><tbody>";
            v.html += formatfilter(chart, v.filter);
            v.html += "                         <td>&nbsp;&nbsp;<a href='#' style='color: #ffffff !important;' class='btn btn-success btn-sm' id='filter_" + chart + "' data-chart='" + chart + "' onclick='CKO.DASHBOARDS.ALLDASHBOARDS.Reports().SelectFilters(this);'>Select Filters</a></td></tr></tbody>"
            v.html += "                     </table> ";
            v.html += "                 </div>";
            v.html += "                 <div class='card-body' id='" + chart + "_panel'></div>";
            v.html += "                 <div class='card-footer' id='tblLegend_" + chart + "'></div>";
            v.html += "             </div>";
            v.html += "         </div> ";
            v.html += "     </div> ";
            v.html += "</div> ";

            //v.html += "<div class='card panel-" + chart + "'>";
            //v.html += "     <div class='card-header'>";
            //v.html += "         <h4 id='panel-title_" + [i] + "'" + "><button class='btn btn-link' type='button' data-toggle='collapse' data-parent='#AllDashboardsReports' data-target='#collapse_" + chart + "'>" + title + "</button></h4>";
            //v.html += "         <table id='filtertable_" + [i] + "'" + " class='allDashboardFilter'><tbody>"; 
            //v.html += formatfilter(chart, v.filter);
            //v.html += "             <td>&nbsp;&nbsp;<a href='#' style='color: #ffffff !important;' class='btn btn-success' id='filter_" + chart + "' data-chart='" + chart + "' onclick='CKO.DASHBOARDS.ALLDASHBOARDS.Reports().SelectFilters(this);'>Select Filters</a></td></tr></tbody>"
            //v.html += "         </table> ";
            //v.html += "     </div> ";
            //v.html += "<div id='" + chart + "'>";
